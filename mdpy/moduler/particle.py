...
from ..unit import default_mass_unit, default_length_unit, default_time_unit
from ..error import *

class Particle:
    def __init__(
        self, particle_id: int, particle_type: str,
        mass=None, position=None, velocity=None,
        ...
        if not isinstance(velocity, type(None)):
            self._velocity = Quantity(velocity, default_length_unit / default_time_unit)
            try:
    ...
    def velocity(self, val):
        val = Quantity(val, default_length_unit/default_time_unit)
        val.value = val.value.reshape(SPATIAL_DIM)
        if val.value.shape[0] == SPATIAL_DIM:
            self._velocity = val
        else:
