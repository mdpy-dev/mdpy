...
def generate_velocity_from_temperature(temperature, particle_masses, seed=None):
    ...
    com_velocity = np.sum(
        particle_masses.astype(np.float64)[:, np.newaxis] * velocities.astype(np.float64), axis=0
    ) / np.sum(particle_masses.astype(np.float64))
