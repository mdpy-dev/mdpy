...
class TileList:
    def __init__(
        ...
        self._num_cells_vec = np.ceil(self._pbc_diag / self._cell_width).astype(
            NUMPY_INT
        )
        ...
        # Kernel
        self._construct_tile_list = cuda.jit(
            nb.void(
        ...
        self._find_tile_neighbors = cuda.jit(
            nb.void(
                NUMBA_INT[::1],  # num_cells_vec
        ...
        # sort
        self._sort_int_matrix = cuda.jit(
            nb.void(
        ...
        self._sort_float_matrix = cuda.jit(
            nb.void(
        ...
        # unsort
        self._unsort_int_matrix = cuda.jit(
            nb.void(
                NUMBA_INT[:, ::1],  # sorted_matrix
                NUMBA_INT[::1],  # matrix_id_mapping_index
                NUMBA_INT[:, ::1],  # unsorted_matrix
        ...
        self._unsort_float_matrix = cuda.jit(
            nb.void(
                NUMBA_FLOAT[:, ::1],  # sorted_matrix
                NUMBA_INT[::1],  # matrix_id_mapping_index
                NUMBA_FLOAT[:, ::1],  # unsorted_matrix
        ...
        # exclusion mask
        self._generate_exclusion_mask_map = cuda.jit(
            nb.void(
    ...
    def set_cutoff_radius(self, cutoff_radius) -> None:
        ...
        if cutoff_radius == 0:
            raise TileListPoorDefinedError(
                "Cutoff radius is poor defined, current value %.3f" % (cutoff_radius)
            )
        ...
        self._num_cells_vec = np.floor(self._pbc_diag / self._cell_width).astype(
            NUMPY_INT
        )
    ...
    def _encode_particles(self, positive_positions: cp.ndarray):
        ...
        cell_index = (
            scaled_int_part[:, 2]
            + scaled_int_part[:, 1] * self._num_cells_vec[2]
        ...
        particle_start_index = cp.zeros_like(num_particles_each_cell, CUPY_INT)
        particle_start_index[1:] = cp.cumsum(num_particles_each_cell)[:-1]
        self._cell_particle_information = cp.stack(
            [particle_start_index, num_particles_each_cell], axis=1
        ).astype(CUPY_INT)
        # cell tile information
        num_tiles_each_ceil = cp.ceil(
            num_particles_each_cell / NUM_PARTICLES_PER_TILE
        ).astype(CUPY_INT)
        self._num_tiles = int(num_tiles_each_ceil.sum())
        self._max_num_tiles_per_cell = int(num_tiles_each_ceil.max())
        tile_start_index = cp.zeros_like(num_tiles_each_ceil, CUPY_INT)
        tile_start_index[1:] = cp.cumsum(num_tiles_each_ceil)[:-1]
        self._cell_tile_information = cp.stack(
            [tile_start_index, num_tiles_each_ceil], axis=1
        ).astype(CUPY_INT)
        # Subcell space filling index
        subcell_index = cp.round(scaled_fraction_part * self._code_range).astype(
            CUPY_INT
        )
        subcell_index = (
            self._device_lookup_table[
                subcell_index[:, 0],
                subcell_index[:, 1],
                subcell_index[:, 2],
            ]
    ...
    def _construct_tile_list_kernel(
        ...
        cell_particle_end_index = (
            cell_particle_start_index + cell_particle_information[cell_index, 1]
        )
        # Get tile information
        tile_index_cur_cell = tile_index - cell_tile_start_index
        tile_particle_start_index = (
            cell_particle_start_index + tile_index_cur_cell * NUM_PARTICLES_PER_TILE
        )
        ...
        local_box = cuda.local.array((SPATIAL_DIM), NUMBA_FLOAT)
        box_max = cuda.local.array((SPATIAL_DIM), NUMBA_FLOAT)
        box_min = cuda.local.array((SPATIAL_DIM), NUMBA_FLOAT)
        tile_particle_index = 0
        sorted_matrix_index = (
            tile_index * NUM_PARTICLES_PER_TILE
        )  # The index of sorted_particle
        # first particle
        ...
        for i in range(SPATIAL_DIM):
            ...
            tile_box[tile_index, i + SPATIAL_DIM] = (
                box_max[i] - box_min[i]
            ) * NUMBA_FLOAT(0.5)
    ...
    def _find_tile_neighbors_kernel(
        ...
        if tile_id >= tile_neighbors.shape[0]:
            return
        central_cell_index = cuda.local.array((SPATIAL_DIM), NUMBA_INT)
        ...
        neighbor_cell_index = cuda.local.array((SPATIAL_DIM), NUMBA_INT)
        local_box = cuda.local.array((7), NUMBA_FLOAT)
            ...
            for j in range(-1, 2):
                ...
                for k in range(-1, 2):
                    ...
                    if neighbor_cell_index[2] >= num_cells_z:
                        ...
                        neighbor_cell_index[2] += num_cells_z
                    cell_index = (
                        neighbor_cell_index[2]
                        + neighbor_cell_index[1] * num_cells_z
                    ...
                    for _ in range(cell_tile_information[cell_index, 1]):
                        ...
                        if is_neighbor:
                            if (
                                math.sqrt(r)
                                - local_box[6]
                                - tile_box[neighbor_tile_id, 6]
                                < cutoff_radius
                            ):
                                tile_neighbors[
                                    tile_id, cur_neighbor_tile_index
                                ] = neighbor_tile_id
    ...
    def update(self, positions: cp.ndarray) -> None:
        ...
        self._tile_list = (
            cp.zeros((self._num_tiles, NUM_PARTICLES_PER_TILE), CUPY_INT) - 1
        )
        self._tile_cell_index = cp.zeros((self._num_tiles, SPATIAL_DIM), CUPY_INT)
        ...
        self._tile_neighbors = (
            cp.zeros(
                (
                    self._num_tiles,
                    int(self._max_num_tiles_per_cell) * NUM_NEIGHBOR_CELLS,
                ),
                CUPY_INT,
            )
            - 1
        ...
        self._tile_neighbors = cp.array(
            self._tile_neighbors[:, : int(cp.max(self._tile_num_neighbors))], CUPY_INT
        )
    ...
    def sort_matrix(self, unsorted_matrix: cp.ndarray) -> cp.ndarray:
        ...
        if matrix_type == CUPY_INT:
            self._sort_int_matrix[block_per_grid, thread_per_block](
                unsorted_matrix, self._matrix_id_mapping_index, sorted_matrix
            )
    ...
    def unsort_matrix(self, sorted_matrix: cp.ndarray) -> cp.ndarray:
        ...
        unsorted_matrix = cp.zeros(
            (self._num_particles, sorted_matrix.shape[0]), matrix_type
        )
        thread_per_block = NUM_PARTICLES_PER_TILE
        block_per_grid = self._num_tiles
        if matrix_type == CUPY_INT:
            self._unsort_int_matrix[block_per_grid, thread_per_block](
                sorted_matrix, self._matrix_id_mapping_index, unsorted_matrix
            )
        elif matrix_type == CUPY_FLOAT:
    ...
    def generate_exclusion_mask_map(
        self, device_excluded_particles: cp.ndarray
    ) -> cp.ndarray:
        ...
        block_per_grid_y = int(
            np.ceil(self._tile_neighbors.shape[1] / NUM_TILES_PER_THREAD)
        )
    ...
    def _generate_exclusion_mask_map_kernel(
        ...
        tile2_particle_index = cuda.shared.array((NUM_PARTICLES_PER_TILE), NUMBA_INT)
        ...
        for tile_index in range(tile_id2_start, tile_id2_end):
            ...
            for particle_index in range(NUM_PARTICLES_PER_TILE):
                ...
                if particle2 == -1 or particle1 == particle2:
                    ...
                    for i in range(SPATIAL_DIM):
                        vec[i] = (
                            tile2_positions[i, particle_index]
...
if __name__ == "__main__":
    ...
    pdb = md.io.PDBParser(
        "/home/zhenyuwei/nutstore/ZhenyuWei/Note_Research/mdpy/mdpy/benchmark/str/medium.pdb"
    )
    psf = md.io.PSFParser(
        "/home/zhenyuwei/nutstore/ZhenyuWei/Note_Research/mdpy/mdpy/benchmark/str/medium.psf"
    )
