...
class Trajectory:
    ...
    def _check_array(self, array: np.ndarray):
        if not isinstance(array, np.ndarray):
            raise TypeError(
                "Array should be numpy.ndarray, instead of %s" % type(array)
            )
        ...
        if num_dims == 2:  # Single frame
            ...
            raise ArrayDimError(
                "The dimensions of array should be 2 or 3 instead of %d" % num_dims
            )
    ...
    def append(self, positions=None, velocities=None, forces=None):
        ...
        if self._contain_positions:
            if isinstance(positions, type(None)):
                raise TrajectoryPoorDefinedError(
                    "This mdpy.core.Trajectory instance contains positions, keyword positions should be specified"
                )
            else:
            ...
            if not isinstance(positions, type(None)):
                raise TrajectoryPoorDefinedError(
                    "This mdpy.core.Trajectory instance does not contain positions, keyword positions should be `None`"
                )

        if self._contain_velocities:
            if isinstance(velocities, type(None)):
                raise TrajectoryPoorDefinedError(
                    "This mdpy.core.Trajectory instance contains velocities, keyword velocities should be specified"
                )
            else:
            ...
            if not isinstance(velocities, type(None)):
                raise TrajectoryPoorDefinedError(
                    "This mdpy.core.Trajectory instance does not contain velocities, keyword velocities should be `None`"
                )

        if self._contain_forces:
            if isinstance(forces, type(None)):
                raise TrajectoryPoorDefinedError(
                    "This mdpy.core.Trajectory instance contains forces, keyword forces should be specified"
                )
            else:
            ...
            if not isinstance(forces, type(None)):
                raise TrajectoryPoorDefinedError(
                    "This mdpy.core.Trajectory instance does not contain forces, keyword forces should be `None`"
                )
        ...
        if len(num_frames) == 0:
            ...
            if len(set(num_frames)) == 1:
                ...
                raise ArrayDimError(
                    "The frames of each array is not same, please check"
                )
