    ...
    def analysis(
        ...
        for index, trajectory in enumerate(trajectory_list):
            ...
            bias_factor_array[index, :] = (
                ...
                self._temperature / KB
            ).value
            num_samples_vec[index] = trajectory.num_frames
        # Iteration solve
        normalization_vec = np.ones([num_trajectories, 1])
        p_est_cur = cv_hist_array.sum(0) / (num_samples_vec * normalization_vec * bias_factor_array).sum(0)
        for i in range(max_iterations):
            normalization_vec = (bias_factor_array * p_est_cur * bin_width).sum(1).reshape([num_trajectories, 1])
            p_est_cur, p_est_pre = cv_hist_array.sum(0) / (num_samples_vec * normalization_vec * bias_factor_array).sum(0), p_est_cur
        free_energy = - (self._temperature * KB * Quantity(np.log(p_est_cur))).convert_to(default_energy_unit).value
        # Output
