...
class RDFAnalyser:
    def __init__(
        ...
        self._bulk_density = check_quantity_value(
            bulk_density, 1 / default_length_unit**3
        )
    ...
    def analysis(self, trajectory: Trajectory, is_dimensionless=True) -> AnalyserResult:
        ...
        selected_matrix_ids_1 = select(trajectory.topology, self._selection_condition_1)
        selected_matrix_ids_2 = select(trajectory.topology, self._selection_condition_2)
        # Analysis
        hist, bin_edge = [], []
        for id1 in selected_matrix_ids_1:
            ...
            for frame in range(trajectory.num_frames):
                ...
                cur_hist_id1, bin_edge = np.histogram(
                    dist, self._num_bins, [0, self._cutoff_radius]
                )
        ...
        bin_center = bin_edge[:-1] + bin_width * 0.5
        if not is_dimensionless:
            cutoff_radius = Quantity(cutoff_radius, default_length_unit)
            bin_center = Quantity(bin_center, default_length_unit)
        ...
        description = {
            ...
            "bin_center": "The bin center of RDF function, unit: default_length_unit",
        }
        data = {
            "mean": mean_hist,
            "std": std_hist,
