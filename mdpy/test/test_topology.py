...
class TestTopology:
    ...
    def test_del_particles(self):
        ...
        self.topology.add_particles(p1, p2, p3, p4)
        self.topology.add_bond([p1, p2])
        self.topology.add_angle([p1, p2, p3])
        self.topology.add_dihedral([p1, p2, p3, p4])
    ...
    def test_add_bond(self):
        ...
        self.topology.add_bond([p2, p1])
        assert self.topology.num_bonds == 1

        self.topology.add_bond([p2, p3])
        assert self.topology.num_bonds == 2

        with pytest.raises(GeomtryDimError):
            self.topology.add_bond([p1, p1, p1])

        with pytest.raises(ParticleConflictError):
            self.topology.add_bond([p1, p1])
    ...
    def test_del_bond(self):
        ...
        self.topology.add_bond([p1, p2])
        assert self.topology.num_bonds == 1
        self.topology.del_bond([p1, p2])
        assert self.topology.num_bonds == 0

        self.topology.add_bond([p1, p2])
        assert self.topology.num_bonds == 1
        self.topology.del_bond([p2, p1])
        assert self.topology.num_bonds == 0

        self.topology.add_bond([p1, p2])
        assert self.topology.num_bonds == 1
        self.topology.del_bond([p2, p3])
        assert self.topology.num_bonds == 1

        with pytest.raises(GeomtryDimError):
            self.topology.del_bond([p1, p1, p1])

        with pytest.raises(ParticleConflictError):
            self.topology.del_bond([p1, p1])
    ...
    def test_add_angle(self):
        ...
        self.topology.add_angle([p3, p2, p1])
        assert self.topology.num_angles == 1

        self.topology.add_angle([p2, p3, p1])
        assert self.topology.num_angles == 2

        with pytest.raises(GeomtryDimError):
            self.topology.add_angle([p1, p1, p1, p2])

        with pytest.raises(ParticleConflictError):
            self.topology.add_angle([p2, p1, p1])
    ...
    def test_del_angle(self):
        ...
        self.topology.add_angle([p1, p2, p3])
        assert self.topology.num_angles == 1
        self.topology.del_angle([p1, p2, p3])
        assert self.topology.num_angles == 0

        self.topology.add_angle([p1, p2, p3])
        assert self.topology.num_angles == 1
        self.topology.del_angle([p3, p2, p1])
        assert self.topology.num_angles == 0

        self.topology.add_angle([p1, p2, p3])
        assert self.topology.num_angles == 1
        self.topology.del_angle([p2, p3, p1])
        assert self.topology.num_angles == 1

        with pytest.raises(GeomtryDimError):
            self.topology.del_angle([p1, p1, p1, p2])

        with pytest.raises(ParticleConflictError):
            self.topology.del_angle([p1, p3, p1])
    ...
    def test_add_dihedral(self):
        ...
        self.topology.add_dihedral([p4, p3, p2, p1])
        assert self.topology.num_dihedrals == 1

        self.topology.add_dihedral([p1, p3, p2, p4])
        assert self.topology.num_dihedrals == 2

        with pytest.raises(GeomtryDimError):
            self.topology.add_dihedral([p1, p1, p2])

        with pytest.raises(ParticleConflictError):
            self.topology.add_dihedral([p2, p1, p1, p3])
    ...
    def test_del_dihedral(self):
        ...
        self.topology.add_dihedral([p1, p2, p3, p4])
        assert self.topology.num_dihedrals == 1
        self.topology.del_dihedral([p1, p2, p3, p4])
        assert self.topology.num_dihedrals == 0

        self.topology.add_dihedral([p1, p2, p3, p4])
        assert self.topology.num_dihedrals == 1
        self.topology.del_dihedral([p4, p3, p2, p1])
        assert self.topology.num_dihedrals == 0

        self.topology.add_dihedral([p1, p2, p3, p4])
        assert self.topology.num_dihedrals == 1
        self.topology.del_dihedral([p2, p4, p3, p1])
        assert self.topology.num_dihedrals == 1

        with pytest.raises(GeomtryDimError):
            self.topology.del_dihedral([p1, p1, p2])

        with pytest.raises(ParticleConflictError):
            self.topology.del_dihedral([p1, p3, p1, p3])
    ...
    def test_add_improper(self):
        ...
        p3 = Particle(2, 'N', molecule_id=0, molecule_type='ASN')
        p4 = Particle(3, 'H', molecule_id=0, molecule_type='ASN')
        self.topology.add_improper([p1, p2, p3, p4])
        assert self.topology.num_impropers == 1

        self.topology.add_improper([p1, p3, p2, p4])
        assert self.topology.num_impropers == 2

        with pytest.raises(GeomtryDimError):
            self.topology.add_improper([p1, p1, p2])

        with pytest.raises(ParticleConflictError):
            self.topology.add_improper([p2, p1, p1, p3])
    ...
    def test_del_dihedral(self):
        ...
        self.topology.add_improper([p1, p2, p3, p4])
        assert self.topology.num_impropers == 1
        self.topology.del_improper([p1, p2, p3, p4])
        assert self.topology.num_impropers == 0

        self.topology.add_improper([p1, p2, p3, p4])
        assert self.topology.num_impropers == 1
        self.topology.del_improper([p4, p3, p2, p1])
        assert self.topology.num_impropers == 1

        with pytest.raises(GeomtryDimError):
            self.topology.del_improper([p1, p1, p2])

        with pytest.raises(ParticleConflictError):
            self.topology.del_improper([p1, p3, p1, p3])