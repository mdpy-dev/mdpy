...
class TestCharmmBondConstraint:
    ...
    def test_bind_ensemble(self):
        ...
        assert (
            ...
            == Quantity(305, kilocalorie_permol).convert_to(default_energy_unit).value
        )
        assert (
            self.constraint._float_parameters[0][1]
            == Quantity(1.3750, angstrom).convert_to(default_length_unit).value
        )
