...
def test_gpu_block_pair_classification():
    ...
    # Unified mask path: all pairs run through the mask-aware kernel, so the
    # single pair list (d_block_pairs) covers every interacting pair and the
    # mask array (d_exclusion_masks) carries one entry per pair slot.
    assert bl.d_block_pairs.shape[0] >= bl.num_block_pairs
    assert bl.d_interacting_atoms.shape[0] >= bl.num_block_pairs * 32
    # masks cover every pair (zero mask = no exclusion = full force)
    assert bl.d_exclusion_masks.shape[0] >= bl.num_block_pairs * 32
