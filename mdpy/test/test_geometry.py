...
def test_get_pbc_bond():
    ...
    position1 = np.array([0, 1, 0]).astype(NUMPY_FLOAT)
    position2 = np.array([0, 0, 0]).astype(NUMPY_FLOAT)
    assert get_pbc_bond(position1, position2, pbc_matrix, pbc_inv) == 1

    position1 = np.array([0, 5, 0]).astype(NUMPY_FLOAT)
    position2 = np.array([0, 0, 0]).astype(NUMPY_FLOAT)
    assert get_pbc_bond(position1, position2, pbc_matrix, pbc_inv) == 5

    position1 = np.array([0, 6, 0]).astype(NUMPY_FLOAT)
    position2 = np.array([0, 0, 0]).astype(NUMPY_FLOAT)
...
def test_get_pbc_angle():
    ...
    p1 = np.array([1, 1, 0]).astype(NUMPY_FLOAT)
    p2 = np.array([0, 0, 0]).astype(NUMPY_FLOAT)
    p3 = np.array([0, 1, 0]).astype(NUMPY_FLOAT)
    angle = get_pbc_angle(p1, p2, p3, pbc_matrix, pbc_inv)
    assert angle == pytest.approx(np.pi / 4)

    p1 = np.array([11, 1, 0]).astype(NUMPY_FLOAT)
    p2 = np.array([0, 0, 0]).astype(NUMPY_FLOAT)
    p3 = np.array([0, 1, 0]).astype(NUMPY_FLOAT)
    angle = get_pbc_angle(p1, p2, p3, pbc_matrix, pbc_inv)
    assert angle == pytest.approx(np.pi / 4)
    p1 = np.array([1, 1, 0]).astype(NUMPY_FLOAT) + 5
    p2 = np.array([0, 0, 0]).astype(NUMPY_FLOAT) + 5
    p3 = np.array([0, 1, 0]).astype(NUMPY_FLOAT) + 5
    angle = get_pbc_angle(p1, p2, p3, pbc_matrix, pbc_inv)
    assert angle == pytest.approx(np.pi / 4)
...
def test_get_pbc_dihedral():
    ...
    p1 = np.array([0, 1, 1]).astype(NUMPY_FLOAT) + 10
    p2 = np.array([0, 0, 0]).astype(NUMPY_FLOAT)
    p3 = np.array([1, 0, 0]).astype(NUMPY_FLOAT)
    p4 = np.array([1, 1, 0]).astype(NUMPY_FLOAT)
    
    dihedral = get_pbc_dihedral(p1, p2, p3, p4, pbc_matrix, pbc_inv)
    assert dihedral == pytest.approx(- np.pi / 4)

    p1 = np.array([0, 0, 0]).astype(NUMPY_FLOAT)
    p2 = np.array([1, 0, 0]).astype(NUMPY_FLOAT)
    p3 = np.array([0, 1, 0]).astype(NUMPY_FLOAT) - 10
    p4 = np.array([0.5, 0.5, -1]).astype(NUMPY_FLOAT)
    
    improper = get_pbc_dihedral(p1, p2, p3, p4, pbc_matrix, pbc_inv)
    assert improper == pytest.approx(- np.pi / 2)

    p1 = np.array([0, 1, 1]).astype(NUMPY_FLOAT)
    p2 = np.array([0, 0, 0]).astype(NUMPY_FLOAT) 
    p3 = np.array([1, 0, 0]).astype(NUMPY_FLOAT)
    p4 = np.array([1, 0, -1]).astype(NUMPY_FLOAT) - 10
