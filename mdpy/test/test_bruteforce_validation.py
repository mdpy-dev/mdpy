"""Validate mdpy GPU block list and forces against brute-force O(N^2) reference.
...
def _extract_block_list_pairs(system):
    import cupy as cp
...
class TestBlockListValidation:
    ...
    def test_neighbor_pair_completeness(self, mdpy_system, ref):
        pair_set, _, _ = _extract_block_list_pairs(mdpy_system)
        ...
        assert len(missing) == 0, (
            f'Block list is missing {len(missing)} non-excluded neighbor pairs '
            f'out of {len(ref_pairs)} total. '
            f'First 5 missing: {list(missing)[:5]}'
        )

    def test_exclusion_masks_correct(self, mdpy_system, ref):
        _, excluded_pair_set, scaled_pair_set = _extract_block_list_pairs(mdpy_system)
        ref_i = ref['neighbor_pairs_i']
        ref_j = ref['neighbor_pairs_j']
        ref_flags = ref['pair_exclusion_flags']
        ...
        assert len(missing_excl) == 0, (
            f'{len(missing_excl)} excluded pairs not marked in block list masks'
        )
        missing_scl = ref_scaled - scaled_pair_set
        assert len(missing_scl) == 0, (
            f'{len(missing_scl)} scaled(1-4) pairs not marked in block list masks'
        )

    def test_atom_mapping_consistency(self, mdpy_system):
        bl = mdpy_system.block_list
        N = mdpy_system.topology.num_particles
        assert     bl.d_sorted_to_pdb.shape[0] == N
        sorted_to_pdb = __import__('cupy').asnumpy(    bl.d_sorted_to_pdb)
