...
class TestCharmmDihedralConstraint:
    ...
    def test_bind_ensemble(self):
        ...
        assert (
            self.constraint._float_parameters[0][0]
            == Quantity(3, kilocalorie_permol).convert_to(default_energy_unit).value
        )
        assert self.constraint._float_parameters[0][1] == Quantity(2).value
        assert self.constraint._float_parameters[0][2] == np.deg2rad(
            Quantity(180).value
        )
    ...
    def test_update(self):
        ...
        theta = get_dihedral(
            [0, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1], is_angular=False
        )
        ...
        energy = self.constraint.potential_energy.get()[0]
        k, n, delta = self.parameters["dihedral"]["CA-NY-CPT-CA"][0]
        theta = get_dihedral([0, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1])
        assert energy == pytest.approx(
            env.NUMPY_FLOAT(k * (1 + np.cos(n * theta - delta)))
        )
