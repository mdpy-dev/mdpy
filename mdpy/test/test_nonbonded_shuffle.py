...
class TestExclusionBlockPairKernel:
    @pytest.fixture
    ...
    def test_kernel_has_block_pair_kernel_name(self, combined_expr):
        source = combined_expr.assemble_exclusion_block_pair_kernel()
        assert 'void exclusion_block_pair_kernel' in source
...
class TestNonbondedForceCompute:
    def test_compute_no_block_pair_counter(self):
        from mdpy.force.nonbonded_force import NonbondedForce
        nf = NonbondedForce(lennard_jones + coulomb)
        assert not hasattr(nf, '_d_block_pair_counter') or nf._d_block_pair_counter is None
