...
class PDBWriter:
    ...
    def _write_header(self):
        ...
        alpha = get_included_angle(
            self._pbc_matrix[0, :], self._pbc_matrix[1, :], is_angular=False
        )
        beta = get_included_angle(
            self._pbc_matrix[0, :], self._pbc_matrix[2, :], is_angular=False
        )
        gamma = get_included_angle(
            self._pbc_matrix[1, :], self._pbc_matrix[2, :], is_angular=False
        )
        header += CRYST1 % (pbc_len[0], pbc_len[1], pbc_len[2], alpha, beta, gamma)
    ...
    def _write_model(self, positions: np.ndarray):
        ...
        for index, particle in enumerate(self._topology.particles):
            if cur_chain_id != particle.chain_id[:1]:
                ...
                model += TER(
                    serial,
                    pre_particle.molecule_type,
                    pre_particle.chain_id,
                    pre_particle.molecule_id,
                )
                serial += 1
            # Serial atomname resname chainid resid x y z 0 0 element
            model += ATOM(
