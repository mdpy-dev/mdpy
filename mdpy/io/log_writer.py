...
class LogWriter:
    ...
    def _get_potential_energy(self, ensemble: Ensemble):
        return (
            "%.3f"
            % Quantity(ensemble.potential_energy, default_energy_unit)
            .convert_to(kilojoule_permol)
            .value
        )
    ...
    def _get_kinetic_energy(self, ensemble: Ensemble):
        return (
            "%.3f"
            % Quantity(ensemble.kinetic_energy, default_energy_unit)
            .convert_to(kilojoule_permol)
            .value
        )
    ...
    def _get_total_energy(self, ensemble: Ensemble):
        return (
            "%.3f"
            % Quantity(ensemble.total_energy, default_energy_unit)
            .convert_to(kilojoule_permol)
            .value
        )
    ...
    def _get_volume(self, ensemble: Ensemble):
        ...
        volume = (
            Quantity(np.abs(volume), default_length_unit**3)
            .convert_to(nanometer**3)
            .value
        )
        return "%.3f" % volume
    ...
    def _get_density(self, ensemble: Ensemble):
        ...
        density = (
            Quantity(mass / volume, default_mass_unit / default_length_unit**3)
            .convert_to(kilogram / meter**3)
            .value
        )
    ...
    def _get_sim_speed(self, cur_step: int):
        ...
        if cpu_time == 0:
            ...
            sim_time = cur_step * self._time_step
            sim_time = (
                Quantity(sim_time, default_time_unit).convert_to(nanosecond).value
            )
