        ...
        for index in range(num_particles):
            particles.append(
                Particle(
                    ...
                    particle_type=self._check_topology_none(
                        bytes.decode(particle_type[index])
                    ),
                    particle_name=self._check_topology_none(
                        bytes.decode(particle_name[index])
                    ),
                    matrix_id=self._check_topology_none(matrix_id[index]),
                    molecule_id=self._check_topology_none(molecule_id[index]),
                    molecule_type=self._check_topology_none(
                        bytes.decode(molecule_type[index])
                    ),
